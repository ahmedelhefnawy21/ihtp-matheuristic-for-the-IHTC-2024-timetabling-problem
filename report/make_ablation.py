...
def main():
    ...
    lines = [r"\begin{table}[!ht]",
             r"\caption{Cumulative ablation, adding the stages in pipeline order on four instances "
             r"that span the size range from the small i04 to the largest i27 (seed 1, so the "
             r"best-of-5 headline objective can be lower). The PAS-MIP gives the largest drop, "
             r"from admissions. The CP-SAT LNS and the exact NRA solve lower every row. The "
             r"ALNS, the descent, and the exact OT solve improve instances where their cost term "
             r"is not already minimal. Construction alone is infeasible on "
