...
\author{Ahmed Elhefnawy \and Weiyi Shen \and Qiaoyan Zeng\\[2pt]
...
\begin{document}
	...
	be postponed. A plan is feasible when it meets eight hard constraints (\Hc1 to \Hc8, listed in part (d)). Among feasible plans, the goal is to minimize a weighted sum of eight soft costs (\Sc1 to
	...
	A single MIP over all four variable families, including the nurse variables, is too large to solve for
	...
	present because our ablation (\S3) adds the stages one at a time in the pipeline's
	fixed order and keeps each only where it lowers the overall cost, dropping anything that did not help.
	\[\small
	...
	\emph{Construction.} We quickly built a first feasible plan. Patients are inserted first
	...
	is below the penalty $W_8$ for leaving it out. A greedy order can fill a surgeon's schedule and leave a mandatory patient unplaced, so we repair \Hc5 with ejection chains, the compound-move idea of \citet{glover1996} (we move the blocking patients to free
	...
	search cannot solve well, but an MIP can\footnote{Because admitting more optionals is a capacity-packing problem \citep{ceschia2011} where one admission needs many patients to move together, local search stalls in local optima while a single MIP optimizes the whole admission packing jointly \citep{maniezzo2010}.}. \citet{ceschia2011} studied patient admission scheduling and derived
	lower bounds for it by relaxation, and our admission model is the assignment form of that problem, matching the code that solves it.\footnote{The one difference: our code computes unscheduled optionals directly as $1-\sum_{r,d}x_{p,d,r}$ rather than with the explicit variable $z_p$; the two are equivalent.} Using the admission variable $x_{p,d,r}$ (created only for
	...
	{\small
		\begin{align*}
			...
			\sum_{p}\delta_p\!\sum_r x_{p,d,r}\le \textstyle\sum_t A_{t,d} && \text{(\Hc3, \Hc4)}
	\end{align*}}%
	...
	search, which repeatedly removes a part of the plan and rebuilds it in a better way. The removal operators (random, worst cost, empty a day,
	empty a theatre, and the related removal of \citet{shaw1998}) each target one cost term. The repair
	operators are greedy insertion, regret-2 insertion (which prioritizes the patient that would lose
	the most by waiting), and randomized greedy. Moves are accepted according to
	the simulated-annealing rule of \citet{kirkpatrick1983}, which accepts some worse plans early and tightens later. Feasibility
	is maintained by construction, so the only violation the search can create is an unadmitted mandatory
	patient, which we handle with a large penalty. A short true-cost descent then makes single room or
	day moves, kept only when the full objective drops. It cleans the secondary costs (\Sc1 age mix and the nurse terms), since it re-scores the whole plan.
	...
	window of days, and re-pack that window with an exact model. This is a large neighborhood search, the
	...
	nurses are re-optimized and only if the full validated objective strictly improves. The search is
	...
	the theatre $y_{p,d,t}$ for each surgery to minimize open theatres (\Sc5) and surgeon transfers
	(\Sc6) subject to \Hc4. Each day has a few surgeries, so this solves to optimality in milliseconds.
	...
	objective $W_2\Sc2+W_3\Sc3+W_4\Sc4$ is minimized over the whole horizon. The greedy nurse assignment
	...
	constraints of the PAS-MIP, and are re-checked on every heuristic move. \Hc3 (surgeon time) is enforced by the PAS-MIP.
	\Hc4 (theatre time) is enforced by the PAS-MIP only in aggregate (total surgery duration per day
	within total theatre capacity); the exact per-theatre bound is enforced when theatres are assigned,
	checked on every placement and move and re-optimized within capacity by the OT-MIP, and confirmed by
	the official validator on every final solution. \Hc5 (all
	...
	test (\S2), so the search optimizes the objective the competition measures. Each soft cost is reduced
	...
	Gurobi is the only commercial solver we use, and it is used under an academic license, as permitted
	...
	Our Python evaluator, which guides the search, matches the official validator component by component on the published i01 reference solution (zero violations, all eight soft costs, and total 3842), and on seven further reference solutions spanning the size range, where we check every hard and soft component. These are feasible published solutions, so this is an agreement test on valid states, not a proof of equivalence for every solution the search visits. It need not be: every solution we report is re-scored by the official validator binary, and we keep the validator's own objective value as the reported number, so our results never rely on the evaluator alone.
	...
	3.12.13, Gurobi 13.0.2, OR-Tools 9.15.6755, and NumPy 2.5.1. The project sets no wall-clock time limit; each solver stops on a deterministic work budget. The full run (all 30 instances, five seeds each, across the ten cores) took about 5.8 hours (20{,}953 seconds) of wall-clock, for the solve and validation, with the certified bounds computed separately. The per-instance runtime in Table~\ref{tab:res} is the solver time summed over the five seeds; the 150 single-threaded runs share the ten cores, so these sum to about 58 hours, roughly ten times the wall-clock. Runtime depends on hardware, so these
	...
	We ran all 30 public instances. The method is feasible on every instance (the official validator reports
	...
	later. The metaheuristic is randomized, so we run five fixed seeds per instance and report the best
	...
	\begin{center}
		\captionof{table}{Results on the 30 public instances. Best-known is the benchmark, and the gap is measured against it. Runtime is the total over five seeds.}\label{tab:res}
	...
	unscheduled patients (\Sc8). Counting admitted optionals directly (not from the weighted \Sc8 cost,
	whose weight varies by instance), we admit 2\,995 against the reference solutions' 2\,933, 62 more
	overall and fewer on a few instances; the weighted \Sc8 is about 2\% lower to match. So the gap is
	not in which optionals we admit. Admission delay (\Sc7) does count against us, at 17\% of the gap,
	but the majority is the nurse layer: continuity (\Sc3) 39\%, under-skill (\Sc2) 30\%, and workload
	...
	two causes, each with a controlled experiment on the same instances and seeds; we report the settled figures below, not the raw sweeps, whose budgets and findings are recorded in \texttt{ihtp/experiments.py}.
	...
	staff. Our lower \Sc8 is partly bought with higher nurse cost. The clearest next steps are to price an estimate of nurse cost into
	the admission objective, and to loop the admission and nurse models rather than run them once.
	\citet{garciatercero2026}, a competition finalist, reach the same conclusion: they flag the link between patient placement and nurse cost as open, and build a dedicated model per subproblem rather than one monolithic model, for speed; our two steps likewise keep the models separate.
	More seeds help only slightly (Appendix~\ref{app:dist}), and longer budgets gave diminishing returns in our profiling, both consistent with the limit being the model coupling rather than compute.
	
	\paragraph{Ablation.}\anote{An ablation is a controlled experiment that measures what one part of a method contributes \citep{fawcett2016}. We add the stages one at a time, in the fixed order the pipeline runs, holding the seed and the per-stage work budgets fixed, and record the objective after each. A stage is kept only when its addition lowers the overall cost. Each stage targets a different cost term (admissions, capacity, theatres, nurses), so its drop cannot be obtained by giving another stage more time; the single seed means the table shows one deterministic path, and the seed spread is reported in Appendix~\ref{app:dist}.} Table~\ref{tab:abl} adds the stages one at a time on representative instances.
	The construction is far from best-known. The PAS-MIP yields the single largest drop because it
	fixes admissions. The ALNS and descent trim the secondary costs. The CP-SAT LNS admits the last
	hard-to-place optional patients, and the exact OT and NRA models remove the final theatre and nurse cost. We dropped
	any component that did not help (for example, a best-fit packing rule we tried early on). We chose these four instances to span the size range, from the small i04 to the largest i27, and add the stages in the pipeline's own fixed order, so neither the subset nor the order is tuned to the outcome.
	...
	bound of our own\anote{A certified optimality gap is the distance between our objective and a proven lower bound on the true optimum, given as a percentage of that bound \citep{wolsey1998}. It differs from the gap to a best-known value, which only compares our result to another heuristic. The lower bound is guaranteed, so the true optimum cannot sit below it, and the real gap to the optimum is at most this figure. We obtain the bound as Gurobi's proven dual bound on a relaxation of the problem, which is a valid lower bound whether or not the relaxation itself is solved to optimality (Appendix~\ref{app:bounds}).}. We solve a relaxation that keeps the admission decision and the two binding
	...
	\citet{ceschia2011} compute relaxation-based lower bounds for patient admission scheduling by the same relaxation argument. This gives a certified gap
	...
	returns the same solution on the same platform and solver versions, and only the runtime
	changes with hardware. We do not claim bit-identical values across a different solver version or CPU
	architecture, since that is a property of the solver and not of our stopping rules. The 30 instances are independent, so they run in parallel across all
	cores without breaking determinism.
	
	\paragraph{Limitations.} These results are for the 30 public instances only, which we also used for development and tuning, with no held-out set, so we do not claim they generalize to the competition's hidden instances or to new ones. The best-known baseline is an upper bound, not a proven optimum, and our certified lower bound is valid but loose, so we locate each true optimum only within the reported interval. The numbers are tied to the solver versions, work budgets, and hardware recorded above, and reproduce under those; a different solver version or CPU architecture may change them. We run five seeds and report the spread in Appendix~\ref{app:dist}, not formal confidence intervals. These are limitations of the evidence, separate from the one open modeling problem, the admission-nurse coupling, which we propose as future work.
	...
	\citep{burke2004}, specialized to the costs and constraints of this competition.
	...
	The entities $e\in\mathcal P\cup\mathcal O$ are the admitted patients and occupants; $r_e$ is the room $e$ occupies,
	$e\in(r,s)$ means $e$ is present in room $r$ during shift $s$, and requirements are indexed from each entity's start
	(for a patient admitted on day $d_0$, stay-shift $j$ is global shift $s=3d_0+j$; occupants are indexed from day~0).
	{\small
		\begin{align*}
	...
	\end{align*}}%
	The greedy nurse assignment warm-starts the NRA-MIP, and the current theatre placement warm-starts the OT-MIP, so each solve begins from a feasible incumbent.
	...
	the objective, and the best-of-5 gap to best-known. Each seeded run is deterministic, so this table
	...
	published reference solutions \citep{ihtc2024site}. The nurse terms \Sc2, \Sc3, \Sc4 make up about 90\% of the net gap (78\% of the gross positive differences), and
	we sit below best-known on the weighted \Sc8, but because that cost mixes per-instance weights, Table~\ref{tab:adm} counts admitted optionals directly (2\,995 versus 2\,933, 62 more).
	\input{component_table.tex}
\input{admissions_table.tex}
	
	\section{Certified lower bounds}\label{app:bounds}
	We bound the true optimum from below by relaxing the exact model: dropping constraints can only lower the optimum, so a relaxation's optimum is a valid lower bound \citep{wolsey1998}, and \citet{ceschia2011} bound patient admission scheduling the same way. Our relaxation is the admission model of \S2 with the room dropped (binary $x_{p,d}$, admit $p$ on day $d$), paired over the whole horizon with the per-day theatre variables of Appendix~\ref{app:models} (binary theatre choice and the open-theatre and surgeon-transfer indicators, under the per-theatre \Hc4). It keeps the admission rules \Hc5 and \Hc6, the surgeon-day \Hc3 and theatre-day \Hc4 time limits, and the four costs those resources drive, \Sc5, \Sc6, \Sc7 and \Sc8, which are all it minimizes. It drops the room and gender assignment (\Hc1, \Hc2, \Hc7) and the bed capacity they imply, and the nurse coverage (\Hc8), and with them the non-negative costs \Sc1 (age mix) and \Sc2 to \Sc4 (nurse). Dropping constraints only enlarges the feasible region and the dropped costs are non-negative, so the relaxation's optimum is a valid lower bound on the true optimum.

	Each relaxation runs on Gurobi with a deterministic $\mathrm{WorkLimit}=120$, $\mathtt{Threads}=1$, $\mathtt{Seed}=0$ and Gurobi's default MIP gap, with no wall-clock limit. The reported bound is $\lfloor\mathrm{ObjBound}\rfloor$, the proven dual bound: tight on i01 to i06, which solve to proven optimality, and the dual bound at the work limit on the other 24 (i07 to i30), still valid but looser, with per-instance status in \texttt{results/bounds.csv}. The aggregate certified gap is pooled over instances, $g=\big(\sum_i \mathrm{ours}_i-\sum_i \mathrm{LB}_i\big)/\sum_i \mathrm{LB}_i$; the table gives each instance's objective, best-known, bound and gap.
