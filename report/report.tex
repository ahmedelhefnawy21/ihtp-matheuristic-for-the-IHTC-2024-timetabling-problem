...
\title{\vspace{-1.2cm}\textbf{A Matheuristic for the Integrated Healthcare\\ Timetabling Problem (IHTC-2024)}}
...
%\begin{abstract}\noindent
%We solve the Integrated Healthcare Timetabling Problem (IHTP) of the IHTC-2024 competition. The
%problem joins three hospital scheduling tasks: patient admission scheduling, surgical case
%planning, and nurse-to-room assignment. Our method is a matheuristic. A constructive heuristic
%and an Adaptive Large Neighborhood Search build and refine a plan, and three small
%mixed-integer programs solve the parts that need to be exact: patient admission,
%operating-theatre choice, and nurse assignment. We justify every design choice by the problem
%structure, by the literature, and by our own ablation. On the 30 public instances the method is
%feasible on all of them, with a total objective 7.8\% above the competition best-known values
%(these are the best results the field has published, not proven optima). We come within 0.5\% of
%best-known on 2 instances and within 3\% on 7. Every solution is checked by the official
%validator, and every number in this report can be reproduced from the code.
%\end{abstract}
...
entries to the competition. The upper layer sets the admission day, room, and theatre
...
search cannot solve well but a MIP can. \citet{ceschia2011} study patient admission scheduling and derive
...
the most by waiting, a standard repair in this framework), and randomised greedy. Moves are accepted by the
...
all their variables and constraints, in Appendix~\ref{app:models}, where the written form matches the code
line for line; the description here covers their purpose and the costs they reduce.
...
the looseness is mostly in the bound. Adding the nurse terms to the bound is the natural way to make
it sharp.
...
reproduces on the same solver versions.
